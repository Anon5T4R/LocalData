...
- **Tabelas tipadas**: texto, texto longo, número, checkbox, data, seleção única,
  seleção múltipla, **relação entre tabelas**, **anexo** (guardado dentro do arquivo)
  e **fórmula** (`{Preço} * {Qtd}`, `IF(...)`, `DAYS(...)`, etc.).
- **Views por tabela**, persistidas na base:
  - **Grade** — virtualizada, edição inline por tipo, redimensionar/ocultar colunas
